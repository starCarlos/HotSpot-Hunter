...
class SQLiteStorageMixin:
    ...
    def _format_full_date(self, date: Optional[str] = None) -> str:
        ...
        用于返回给用户的数据，确保返回完整日期而不是月份
        
        Args:
            date: 日期字符串（YYYY-MM-DD 或 YYYY-MM），为 None 则使用当前日期
        ...
        if date:
            ...
            if len(date) >= 10:
                return date[:10]  # 取前10个字符
            # 如果是月份格式 (YYYY-MM)，返回该月第一天
            elif len(date) == 7:
                return f"{date}-01"
            else:
                return date
    ...
    def _get_today_all_data_impl(self, date: Optional[str] = None) -> Optional[NewsData]:
        ...
        try:
            ...
            # 返回用户查询的完整日期
            crawl_date = self._format_full_date(date)
    ...
    def update_news_importance(
        ...
        try:
            ...
            print(f"[存储] 更新重要性失败: {e}")
            return False

    def get_news_importance(
        self,
        title: str,
        platform_id: str,
        date: Optional[str] = None,
        ...
        try:
            ...
            cursor.execute("""
                SELECT importance FROM news_items
                WHERE title = ? AND platform_id = ?
                LIMIT 1
            """, (title, platform_id))
            ...
            print(f"[存储] 获取重要性失败: {e}")
            return None

    def batch_get_news_importance(
        self,
